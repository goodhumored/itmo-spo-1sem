...
Operand create_string_literal_operand(const char *str) {
  Operand op = {0};
  op.kind = OPND_STRING_LITERAL;
  op.value.name = str ? strdup(str) : NULL;
  return op;
}
...
Operation *create_operation(OperationType type, Operand op1, Operand op2,
                            Operand dest) {
...
void add_operation_to_block(BasicBlock *block, Operation *op) {
  ...
  if (block->op_count >= block->max_ops) {
    ...
    block->operations =
        realloc(block->operations, block->max_ops * sizeof(Operation *));
  }
...
void add_successor(BasicBlock *from, BasicBlock *to) {
  if (from->successor_count >= from->max_successors) {
    from->max_successors =
        from->max_successors == 0 ? 2 : from->max_successors * 2;
    from->successors =
        realloc(from->successors, from->max_successors * sizeof(BasicBlock *));
  }
...
void add_predecessor(BasicBlock *to, BasicBlock *from) {
  ...
  if (to->predecessor_count >= to->max_predecessors) {
    to->max_predecessors =
        to->max_predecessors == 0 ? 2 : to->max_predecessors * 2;
    to->predecessors =
        realloc(to->predecessors, to->max_predecessors * sizeof(BasicBlock *));
  }
...
void add_argument(Function *func, const char *name, const char *type) {
  ...
  // Stack layout for arguments (caller pushes arguments):
  // [BP+8]   <- first argument (index 0)
  // [BP+12]  <- second argument (index 1)
  // ...
  arg->offset = 8 + arg->index * 4;
...
void add_local_var(Function *func, const char *name, const char *type) {
  ...
  var->offset = - 4 - (func->local_var_count * 4);
