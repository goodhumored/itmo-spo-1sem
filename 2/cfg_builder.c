...
typedef struct BuildContext {
  ...
  OperationType last_cmp_type; // для хранения типа последнего сравнения (OP_LT,
                               // OP_GT и т.д.)
} BuildContext;

static int get_next_temp(BuildContext *ctx) { return ctx->next_temp_id++; }
...
static Operation *create_const_op(int64_t val) {
  ...
  return create_operation(OP_CONST, const_op, create_undef_operand(),
                          create_undef_operand());
}

static Operation *create_binary_op(const char *op_str, Operand left,
                                   Operand right, Operand dest,
                                   ASTNode *ast_node) {
  OperationType type;
  ...
  Operation *op = create_operation(type, left, right, dest);
  op->ast_node = ast_node; // Save AST node for string literal collection
  return op;
}
...
static Operation *create_store_op(Operand var, Operand value,
                                  ASTNode *ast_node) {
  Operation *op =
      create_operation(OP_STORE, value, var, create_undef_operand());
  op->ast_node = ast_node; // Save AST node for string literal collection
  return op;
}
...
static Operation *create_call_op(const char *func_name, Operand dest,
                                 ASTNode *ast_node) {
  Operand func_label = create_label_operand(func_name);
  Operation *op =
      create_operation(OP_CALL, func_label, create_undef_operand(), dest);
...
static Operation *create_jmp_op(const char *target) {
  ...
  return create_operation(OP_JMP, target_label, create_undef_operand(),
                          create_undef_operand());
}

static BasicBlock *process_statement(BuildContext *ctx, ASTNode *stmt);
static BasicBlock *process_statement_list(BuildContext *ctx,
                                          ASTNode *stmt_list);
static BasicBlock *process_if_statement(BuildContext *ctx, ASTNode *if_stmt);
static BasicBlock *process_while_statement(BuildContext *ctx,
                                           ASTNode *while_stmt);
static BasicBlock *process_do_while_statement(BuildContext *ctx,
                                              ASTNode *do_stmt);
static BasicBlock *process_break_statement(BuildContext *ctx,
                                           ASTNode *break_stmt);
...
static Operand process_expression(BuildContext *ctx, ASTNode *expr) {
  ...
  if (strcmp(type, "literal") == 0 && expr->value) {
    ...
    if (expr->value[0] == '"' && expr->value[strlen(expr->value) - 1] == '"') {
            // return create_string_literal_operand(expr->value);
            return create_const_operand((uint32_t)expr->value[1]);
    }
    int64_t val;
  ...
  if (strcmp(type, AST_BINARY) == 0) {
    ...
    if (strcmp(op, "=") == 0 && expr->child_count >= 2) {
      ...
      Operation *store_op = create_store_op(left, right, expr);
      ...
      Operation *bin_op = create_binary_op(op, left, right, dest, expr);
...
static void extract_local_vars(CFG *cfg, ASTNode *block) {
  if (!block || strcmp(block->type, "block") != 0)
    return;
...
static BasicBlock *process_if_statement(BuildContext *ctx, ASTNode *if_stmt) {
  ...
  Operation *cjmp_op = create_operation(
      OP_CJMP, cond_result, create_undef_operand(), create_undef_operand());
  ...
  // Добавляем безусловный переход к merge если есть else или если не все пути
  // заканчиваются
...
static BasicBlock *process_while_statement(BuildContext *ctx,
                                           ASTNode *while_stmt) {
  ASTNode *condition =
      while_stmt->child_count > 0 ? while_stmt->children[0] : NULL;
  ...
  BasicBlock *body_result_block = NULL;
  if (body) {
    ctx->current_block = loop_body;
    body_result_block = process_statement(ctx, body);
  }
  ...
  if (body_result_block && body_result_block != loop_header) {
    add_successor(body_result_block,loop_header);
  }
...
static BasicBlock *process_statement(BuildContext *ctx, ASTNode *stmt) {
  if (!stmt || !ctx->current_block)
    return ctx->current_block;

  if (strcmp(stmt->type, AST_IF) == 0) {
    return process_if_statement(ctx, stmt);
  } else if (strcmp(stmt->type, AST_WHILE) == 0) {
    return process_while_statement(ctx, stmt);
  } else if (strcmp(stmt->type, AST_DO) == 0) {
    return process_do_while_statement(ctx, stmt);
  } else if (strcmp(stmt->type, AST_BREAK_STMT) == 0) {
        printf("BREEEEEEEEEEAAAAK!!!!\n");
    return process_break_statement(ctx, stmt);
  } else if (strcmp(stmt->type, AST_BLOCK) == 0) {
    return process_statement_list(ctx, stmt);
      ...
      for (int i = 0; i < var_list->child_count; i++) {
        ...
        if (var_item && strcmp(var_item->type, "varInit") == 0) {
          ...
          if (var_item->value && var_item->child_count > 0) {
            ...
            Operation *store_op = create_store_op(var_dest, init_val, var_item);
            emit_op(ctx, store_op);
          }
        }
...
static BasicBlock *process_statement_list(BuildContext *ctx,
  ...
  BasicBlock *last_block = ctx->current_block;
  for (int i = 0; i < stmt_list->child_count; i++) {
    ...
    BasicBlock *result = process_statement(ctx, stmt_list->children[i]);
    if (result) {
      last_block = result;
    }
  }
