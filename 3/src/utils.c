...
void print_data(VMProgram *program, FILE *out) {
  // Print data section
  if (program->data_item_count > 0) {
    fprintf(out, "[section _data, RAM]\n");
...
void print_code_section(VMProgram *program, FILE *out) {
      // Print code section with offset address
  fprintf(out, "[section name=_code, bank=RAM]\n");

  fprintf(out, "    MOV SP, 0xFFFF\n");
...
void print_program_listing(VMProgram *program, FILE *out) {
  ...
  print_code_section(program, out);
  print_data(program, out);
}
